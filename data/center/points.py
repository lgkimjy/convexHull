...
sc3 = plt.scatter(0.3, 0.7, color='cyan', alpha=1, s=100)
...
# foot position
sc4 = plt.scatter(0.1, 0.0, color='black', label='rfoot', alpha=1, s=100)
plt.text(0.1 + 0.02, 0.0 + 0.02, "LFoot", fontsize=11, color='black')
sc5 = plt.scatter(-0.1,0.0,  color='black', label='lfoot', alpha=1, s=100)
plt.text(-0.1 + 0.02, 0.0 + 0.02, "RFoot", fontsize=11, color='black')

# average feasible points
sc3 = plt.scatter(y3[0], x3[0], color='green', label='average_feasible_points : ' + str(len(x3)), alpha=1, s=150)

# # feasible p CoM
sc8 = plt.scatter(y4, x4, color='purple', label='feasible_p_CoM : ' + str(len(x4)), alpha=1, s=150)
...
handles = [
    plt.Line2D([], [], marker='o', color='w', markerfacecolor='blue', markersize=8, label='sample_sets : '+str(len(x1))),
    plt.Line2D([], [], marker='o', color='w', markerfacecolor='red', markersize=8, label='feasible_sets : '+str(len(x2))),
    plt.Line2D([], [], marker='o', color='w', markerfacecolor='purple', markersize=8, label='p_CoM sets under feasiblility'),
    plt.Line2D([], [], marker='o', color='w', markerfacecolor='green', markersize=8, label='average_feasible_sets'),
    # plt.Line2D([], [], marker='o', color='w', markerfacecolor='cyan', markersize=8, label='left_hand_target')
    plt.Line2D([], [], marker='o', color='w', markerfacecolor='cyan', markersize=8, label='left_hand_target'),
    plt.Line2D([], [], color='orange', linewidth=2, label='convex_hull'),
